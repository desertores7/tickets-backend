...
import { QUEUE_NAMES, SendOrderTicketsEmailJobData } from '@config/redis/bull-jobs.types';
...
export class SendOrderTicketsEmailProcessor extends WorkerHost {
  ...
  async process(job: Job<SendOrderTicketsEmailJobData>): Promise<void> {
    if (job.name !== 'send-order-tickets-email') return;
