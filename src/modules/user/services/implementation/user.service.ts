...
export class UserService implements IUserService {
  ...
  async createUser(data: IUserCreate): Promise<void> {
    ...
    try {
      ...
      // Determinar qué rol asignar: el proporcionado o "Vendedor" por defecto
      let roleUuidToAssign: string = operatorRole.uuid; // Por defecto "Vendedor"

      if (data.roleUuid) {
        // Si se proporciona un roleUuid, verificar que existe usando RoleService
        try {
          const role = await this.roleService.getRoleId(data.roleUuid);
          roleUuidToAssign = role.uuid;
        } catch (error) {
      ...
      await this.roleService.assignRoleToUser(userUuid, roleUuidToAssign, userUuid, queryRunner, true);
