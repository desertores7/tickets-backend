...
- **Modo de venta por tanda** (`BR-SALE-010`, `ticket_type.saleMode`): `general` (default, por cantidad), `per_person` (lugares dentro de una mesa/palco) o `whole_unit` (unidad completa: el precio es el de la mesa entera y genera `admissionsPerUnit` entradas). Regla en `event/services/core/ticket-type-sale-mode.ts`; no se cambia con ventas. La compra todavía no registra la unidad elegida (pendiente)
