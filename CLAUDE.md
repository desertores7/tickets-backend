...
- **Extracción del flyer con IA**: `gpt-4o` RECHAZA afiches con caras de personas reales (`refusal: I can't assist with that`) aunque solo se le pida transcribir. La extracción reintenta en modo OCR estricto y después con `EVENT_AI_EXTRACT_FALLBACK_MODEL` (vacío = sin respaldo; el mapa usa `gpt-5.6-sol` y no rechaza)
